...
export default async function handler(req: any, res: any) {
  ...
  try {
    const { id } = req.query;
    if (!id) {
      return res.status(400).json({ error: 'Coverage ID / Insurance No is required' });
    }
    ...
    return res.status(error.response?.status || 500).json(error.response?.data || { error: "Failed to search coverage" });
  }
}
