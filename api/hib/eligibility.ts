...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const targetUrl = `${baseUrl}api/api_fhir/EligibilityRequest/`;
    ...
    return res.status(error.response?.status || 500).json(error.response?.data || { error: "Failed to check eligibility" });
  }
}
