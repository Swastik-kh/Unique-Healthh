...
const oldBlockStart = `{isSuperAdmin && mappedOrgsInfo.length > 0 && (`;
const oldBlockEnd = `      )}`;

let startIdx = content.indexOf(oldBlockStart);
let endIdx = content.indexOf(oldBlockEnd, startIdx);

if (startIdx !== -1 && endIdx !== -1) {
    // Find the actual end of the block (the last brace before the final part)
    // Actually our previous tail shows many braces.
    // Let's just find the first occurrence of `      )}` after the start.
    let blockEndIdx = endIdx + oldBlockEnd.length;
    let finalContent = content.substring(0, startIdx) + newSummaryUI + content.substring(blockEndIdx);
