...
export const GESIReport: React.FC<GESIReportProps> = ({
  ...
  const pushToDHIS2 = async () => {
    ...
    try {
      ...
      // Map GESI Data
      // For now, mapping some key indicators if they have mappings
      // We can iterate through CASTE_GROUPS and reportData to generate data values
      // based on source keys like: GESI_KHOP_F_DALIT, GESI_KHOP_M_DALIT, etc.
      
      CASTE_GROUPS.forEach(caste => {
        const row = reportData[caste.id];
        const casteKey = caste.id.toUpperCase().replace('/', '_');

        // Mapping helper for each cell
        const mapCell = (keySuffix: string, value: number) => {
          const mapping = getDhis2CellMapping(`GESI_${keySuffix}_${casteKey}`, generalSettings, { dataElement: 'NOT_MAPPED', categoryOptionCombo: '' });
        ...
        mapCell('KHOP_F', row.khop_f);
        mapCell('KHOP_M', row.khop_m);
        mapCell('UNDERWEIGHT_F', row.underweight_f);
        mapCell('UNDERWEIGHT_M', row.underweight_m);
        mapCell('DELIVERY', row.delivery);
        mapCell('ABORTION', row.abortion);
        mapCell('HIV_O', row.hiv_o);
        mapCell('HIV_F', row.hiv_f);
        mapCell('HIV_M', row.hiv_m);
        mapCell('LEPROSY_F', row.leprosy_f);
        mapCell('LEPROSY_M', row.leprosy_m);
        mapCell('TB_F', row.tb_f);
        mapCell('TB_M', row.tb_m);
        mapCell('DISCHARGE_F', row.discharge_f);
        mapCell('DISCHARGE_M', row.discharge_m);
        mapCell('GBV_F', row.gbv_f);
        mapCell('GBV_M', row.gbv_m);
      });
