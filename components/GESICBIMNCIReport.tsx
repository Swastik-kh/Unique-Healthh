...
import { ServiceSeekerRecord, CBIMNCIRecord } from '../types';
import { useReactToPrint } from 'react-to-print';
// @ts-ignore
import NepaliDate from 'nepali-date-converter';
import { NepaliDatePicker } from './NepaliDatePicker';

interface GESICBIMNCIReportProps {
...
export const GESICBIMNCIReport: React.FC<GESICBIMNCIReportProps> = ({
  currentFiscalYear,
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: 'GESI_CBIMNCI_Report',
  });
  ...
  }, [cbimnciRecords, currentFiscalYear, reportType, selectedDate, selectedMonth, selectedQuarter, selectedHalfYear]);

  const reportData = useMemo(() => {
    const infantData: Record<string, { m: number; f: number }> = {};
    const childData: Record<string, { m: number; f: number }> = {};
    
    CASTE_GROUPS.forEach(c => {
      infantData[c.id] = { m: 0, f: 0 };
      childData[c.id] = { m: 0, f: 0 };
    });
    ...
    filteredRecords.forEach(record => {
      ...
      if (patient) {
        ...
        if (record.moduleType === 'Infant') {
          if (infantData[caste]) infantData[caste][gender]++;
        } else {
          if (childData[caste]) childData[caste][gender]++;
        }
      }
    });
    return { infant: infantData, child: childData };
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-slate-200 pb-4 no-print gap-4">
        <div>
          <h2 className="text-xl font-bold text-slate-800 font-nepali flex items-center gap-2">
            ...
            GESI CBIMNCI सेवा रिपोर्ट
          </h2>
          <p className="text-sm text-slate-500">नवशिशु तथा बालरोगको एकीकृत व्यवस्थापन सेवा लिएका बच्चा संख्या</p>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <div className="overflow-x-auto p-8" ref={componentRef}>
          ...
          <div className="header-title mb-2 text-center font-bold text-xl">
            नवशिशु तथा बालरोगको एकीकृत व्यवस्थापन सेवा लिएका बच्चा संख्या
          </div>
          ...
          <div className="space-y-10">
            {/* Section 1: Under 2 Months */}
            <div>
              <h3 className="text-lg font-bold mb-3 text-slate-800 border-b pb-2">१. २ महिना मुनिका (०-५९ दिन)</h3>
              <table className="w-full border-collapse border border-slate-300 text-center">
                <thead>
                  <tr className="bg-slate-50">
                    ...
                    <th colSpan={3} className="border border-slate-300 p-4">सेवा लिएका बच्चा संख्या</th>
                  </tr>
                  <tr className="bg-slate-50">
                    <th className="border border-slate-300 p-3">बालिका (म.)</th>
                    <th className="border border-slate-300 p-3">बालक (पु.)</th>
                    <th className="border border-slate-300 p-3">जम्मा</th>
                  </tr>
                </thead>
                <tbody>
                  {CASTE_GROUPS.map(caste => {
                    const row = reportData.infant[caste.id];
                    return (
                ...
                <tfoot className="bg-slate-100 font-bold">
                  <tr>
                    ...
                    <td className="border border-slate-300 p-4">{(Object.values(reportData.infant) as { m: number; f: number }[]).reduce((acc, curr) => acc + curr.f, 0)}</td>
                    <td className="border border-slate-300 p-4">{(Object.values(reportData.infant) as { m: number; f: number }[]).reduce((acc, curr) => acc + curr.m, 0)}</td>
                    <td className="border border-slate-300 p-4">{(Object.values(reportData.infant) as { m: number; f: number }[]).reduce((acc, curr) => acc + curr.f + curr.m, 0)}</td>
                  </tr>
                </tfoot>
              </table>
            </div>

            {/* Section 2: 2 to 59 Months */}
            <div>
              <h3 className="text-lg font-bold mb-3 text-slate-800 border-b pb-2">२. २ महिना देखि ५९ महिना सम्मका</h3>
              <table className="w-full border-collapse border border-slate-300 text-center">
                <thead>
                  <tr className="bg-slate-50">
                    <th rowSpan={2} className="border border-slate-300 p-4 w-1/3">जात/जाती</th>
                ...
                <tbody>
                  {CASTE_GROUPS.map(caste => {
                    const row = reportData.child[caste.id];
                    return (
                      <tr key={caste.id} className="hover:bg-slate-50 transition-colors">
                        <td className="border border-slate-300 p-4 text-left font-medium">{caste.name}</td>
                        <td className="border border-slate-300 p-4">{row.f || '-'}</td>
                ...
                <tfoot className="bg-slate-100 font-bold">
                  <tr>
                    <td className="border border-slate-300 p-4 text-left">कुल जम्मा</td>
                    <td className="border border-slate-300 p-4">{(Object.values(reportData.child) as { m: number; f: number }[]).reduce((acc, curr) => acc + curr.f, 0)}</td>
                    <td className="border border-slate-300 p-4">{(Object.values(reportData.child) as { m: number; f: number }[]).reduce((acc, curr) => acc + curr.m, 0)}</td>
                    <td className="border border-slate-300 p-4">{(Object.values(reportData.child) as { m: number; f: number }[]).reduce((acc, curr) => acc + curr.f + curr.m, 0)}</td>
                  </tr>
                </tfoot>
              </table>
            </div>
