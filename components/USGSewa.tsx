...
export const USGSewa: React.FC<USGSewaProps> = ({
  records,
  ...
  const [editingRecord, setEditingRecord] = useState<USGRecord | null>(null);
  ...
  const handlePatientSelect = (patient: ServiceSeekerRecord) => {
    ...
    const opdRef = opdRecords.find(r => r.serviceSeekerId === patient.id && (r.investigation?.toLowerCase().includes('usg') || r.investigation?.toLowerCase().includes('u.s.g.')));
    const erRef = emergencyRecords.find(r => r.serviceSeekerId === patient.id && (r.investigation?.toLowerCase().includes('usg') || r.investigation?.toLowerCase().includes('u.s.g.')));
    const cbimnciRef = cbimnciRecords.find(r => r.serviceSeekerId === patient.id && (r.investigation?.toLowerCase().includes('usg') || r.investigation?.toLowerCase().includes('u.s.g.')));
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const record: USGRecord = {
      id: editingRecord?.id || `USG-${Date.now()}`,
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold text-gray-800 font-nepali">यु.एस.जी. सेवा (USG Service)</h2>
          <p className="text-sm text-gray-500">यु.एस.जी. रेकर्डिङ र व्यवस्थापन</p>
        </div>
        <button
          onClick={() => {
            setEditingRecord(null);
      ...
      {isFormOpen && (
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {referralInfo && (
              <div className="bg-indigo-50 border border-indigo-200 p-4 rounded-xl flex items-start gap-3 animate-in fade-in slide-in-from-top-2">
                <AlertCircle className="text-indigo-600 mt-1 shrink-0" size={20} />
                <div>
                  <h4 className="font-bold text-indigo-800 text-sm">USG सिफारिस भेटियो!</h4>
                  <p className="text-xs text-indigo-700 mt-1">
                    यो सेवाग्राहीलाई <strong>{referralInfo.source}</strong> बाट यु.एस.जी.को लागि सिफारिस गरिएको छ।
                  </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t">
              <div className="space-y-2">
                <label className="text-sm font-medium text-gray-700">यु.एस.जी. प्रकार (USG Type)</label>
                <input
                  type="text"
                  required
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  className="w-full p-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500"
                  placeholder="यु.एस.जी. रिपोर्टको संक्षिप्त विवरण..."
                />
              </div>
