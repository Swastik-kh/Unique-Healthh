...
export const LabProtsahanBharpaiModal: React.FC<LabProtsahanBharpaiModalProps> = ({
  ...
  const bharpaiRows: BharpaiRow[] = useMemo(() => {
    ...
    (protsahanByReferrer || []).forEach((ref) => {
      const grossAmt = typeof ref.netLabAmount === 'number' ? ref.netLabAmount : (typeof (ref as any).grossLabAmount === 'number' ? (ref as any).grossLabAmount : null);
      const incAmt = Number(ref.referrerShare ?? ref.incentiveAmount ?? ref.totalIncentive ?? 0) || 0;
