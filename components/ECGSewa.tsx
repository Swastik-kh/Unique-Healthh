...
interface ECGSewaProps {
  ...
  onSave: (record: ECGRecord) => void;
  onDelete: (id: string) => void;
  currentFiscalYear: string;
}

export const ECGSewa: React.FC<ECGSewaProps> = ({
  records,
  ...
  const [editingRecord, setEditingRecord] = useState<ECGRecord | null>(null);
  ...
  const [formData, setFormData] = useState<Partial<ECGRecord>>({
  ...
  const handlePatientSelect = (patient: ServiceSeekerRecord) => {
    ...
    const opdRef = opdRecords.find(r => r.serviceSeekerId === patient.id && (r.investigation?.toLowerCase().includes('ecg') || r.investigation?.toLowerCase().includes('e.c.g.')));
    const erRef = emergencyRecords.find(r => r.serviceSeekerId === patient.id && (r.investigation?.toLowerCase().includes('ecg') || r.investigation?.toLowerCase().includes('e.c.g.')));
    const cbimnciRef = cbimnciRecords.find(r => r.serviceSeekerId === patient.id && (r.investigation?.toLowerCase().includes('ecg') || r.investigation?.toLowerCase().includes('e.c.g.')));
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const record: ECGRecord = {
      id: editingRecord?.id || `ECG-${Date.now()}`,
      ...
      ecgType: formData.ecgType || 'Resting',
        ...
        {patientsOnQueue.length > 0 && (
          <div className="bg-primary-50 p-4 rounded-xl border border-primary-100 animate-in fade-in">
            <h3 className="text-xs font-bold text-primary-800 uppercase mb-3 flex items-center gap-2">
              ...
              पर्खिरहेका ई.सी.जी. बिरामीहरू (Patients on Queue): {patientsOnQueue.length}
            </h3>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {referralInfo && (
              <div className="bg-emerald-50 border border-emerald-200 p-4 rounded-xl flex items-start gap-3 animate-in fade-in slide-in-from-top-2">
                ...
                <div>
                  <h4 className="font-bold text-emerald-800 text-sm">ECG सिफारिस भेटियो!</h4>
                  <p className="text-xs text-emerald-700 mt-1">
                    यो सेवाग्राहीलाई <strong>{referralInfo.source}</strong> बाट ई.सी.जी.को लागि सिफारिस गरिएको छ।
                  </p>
